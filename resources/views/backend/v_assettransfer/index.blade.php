...
<div class="card shadow-sm border-0">
  <div class="card-body">
    <h4 class="card-title mb-3">📄 Daftar Formulir Peralihan Asset IT</h4>
    <hr>

    <div class="mb-3">
      <a href="{{ route('backend.assettransfer.create', 2) }}" class="btn btn-primary btn-sm">
        <i class="fas fa-plus"></i> Add
      </a>
    </div>

    <div class="table-responsive">
        <table id="transferTable" class="table table-hover table-bordered align-middle">
            <thead class="table-custom" style="background-color:#dcdff5ff; color:#000; font-weight:normal; font-size:14px;">
            <tr>
                <th class="text-center">No</th>
                <th class="text-center">No Dokumen</th>
                <th class="text-center">Tanggal Peralihan</th>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="d-flex justify-content-center px-2 py-1" style="gap:5px; font-size:11px;">
                        <a href="{{ route('backend.assettransfer.show', $row->id) }}" 
                           class="btn btn-outline-secondary btn-sm d-flex align-items-center justify-content-center" 
                           style="width:36px; height:30px; font-size:11px;" title="Detail">
                            <i class="fas fa-eye" style="font-size:11px;"></i>
                        </a>

                        <a href="{{ route('backend.assettransfer.cetak', $row->id) }}" 
                        ...
                        <a href="{{ route('backend.assettransfer.edit', $row->id) }}" 
                           class="btn btn-outline-primary btn-sm d-flex align-items-center justify-content-center" 
                           style="width:36px; height:30px; font-size:11px;" title="Edit">
                            <i class="far fa-edit" style="font-size:11px;"></i>
                        </a>

                        <form action="{{ route('backend.assettransfer.destroy', $row->id) }}" method="POST" class="d-inline-flex align-items-center m-0 p-0">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="btn btn-outline-danger btn-sm d-flex align-items-center justify-content-center show_confirm" 
                                    ...
                                    data-konf-delete="{{ $row->asset_brand ?? $row->id }}" title="Hapus">
