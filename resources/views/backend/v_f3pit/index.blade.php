...
<div class="card shadow-sm border-0">
  <div class="card-body">
    <h4 class="card-title mb-3">📝 Daftar Formulir Permintaan Perbaikan Perangkat IT (F3PIT)</h4>
    <hr>

    <div class="mb-3">
      <a href="{{ route('backend.f3pit.create') }}" class="btn btn-primary btn-sm">
        <i class="fas fa-plus"></i> Add
      </a>
    </div>

    <div class="table-responsive">
        <table id="f3pitTable" class="table table-hover table-bordered align-middle">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="d-flex justify-content-center px-2 py-1" style="gap:5px; font-size:11px;">
                        ...
                        <form action="{{ route('backend.f3pit.destroy', $row->id) }}" method="POST" class="d-inline-flex align-items-center m-0 p-0">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="btn btn-outline-danger btn-sm d-flex align-items-center justify-content-center show_confirm" 
                                    ...
                                    data-konf-delete="{{ $row->departement ?? $row->id }}" title="Hapus">
